...
# 외부 Redis 서버에 연결
redis_client = redis.Redis(host=redis_host, port=redis_port, password=redis_password, charset="utf-8", decode_responses=True,db=0)
...
def confirm():
    ...
    if email != None:
        ...
        redis_client.expire(value, 180)
        print(email)
        subprocess.run(["python3", "wait.py", email])
    else:
        otp_entry.delete(0, tk.END)
...
otp_entry = tk.Entry(otp, font=("NanumGothic", 30))
